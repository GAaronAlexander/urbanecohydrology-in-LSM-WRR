...
MODULE MODULE_SF_NOAHMPLSM
  ...
  private ::                         WDFCND2
  private ::               SSTEP
  private ::       SOILWATER_greenroof !Added by Aaron A.
  private ::       GROUNDWATER
...
!== begin noahmp_sflx ==============================================================================
  ...
  SUBROUTINE NOAHMP_SFLX (parameters, &
                   ...
                   SHDFAC  , SHDMAX  , VEGTYP  , ICE     , IST     , CROPTYPE, & ! IN : Vegetation/Soil characteristics ! Edited by G. Aaron Alexander
	           ...
	           PRCPCONV, PRCPNONC, PRCPSHCV, PRCPSNOW, PRCPGRPL, PRCPHAIL, & ! IN : Forcing
                   ...
                   SMCWTD  ,DEEPRECH , RECH    , BTRANI_dummy,                 & ! IN/OUT : added by Aaron A.
                   GECROS1D,SMC_dummy, SH2O_dummy,                                                   & ! IN/OUT :
		   Z0WRF   , &
                   FSA     , FSR     , FIRA    , FSH     , SSOIL   , FCEV    , & ! OUT :
                   FGEV    , FCTR    , ECAN    , ETRAN   , EDIR    , TRAD    , & ! OUT :
                   TGB     , TGV     , T2MV    , T2MB    , Q2V     , Q2B     , & ! OUT :
                   RUNSRF  , RUNONSRF, RUNSUB  , APAR    , PSN     , SAV     , SAG     , & ! OUT : !Aaron A.
		   ...
		   CHLEAF  , CHUC    , CHV2    , CHB2    , FPICE   , PAHV    , &
                  ...
                  , NSOIL_GR, DETENTION_STORAGE,FAREA,VOL_FLUX_SM,VOL_FLUX_RUNON )
...
! input
  ...
  REAL, DIMENSION(       1:NSOIL), INTENT(INOUT)    :: ZSOIL  !layer-bottom depth from soil surf (m) ! changed by Aaron A. 
...
! prognostic variables
  ...
  REAL,                            INTENT(INOUT) :: RUNONSRF !runon in mm/s Aaron A.
  REAL, DIMENSION(       1:NSOIL), INTENT(INOUT) :: BTRANI_dummy
  REAL, DIMENSION(1:60)          , INTENT(INOUT) :: gecros1d !  gecros crop
  REAL, DIMENSION( 1:NSOIL), INTENT(INOUT) :: SMC_dummy !these are added by Aaron A. They contain the actual soil moisture of the grid ALWAYS
  REAL, DIMENSION( 1:NSOIL), INTENT(INOUT) :: SH2O_dummy !these are added by Aaron A. They contain the actual soil moisture of the grid ALWAYS
! output
...
!more local variables for precip heat MB
  ...
  LOGICAL                             :: GREEN_ROOF ! add green roof logical for ease Aaron A.
  REAL :: FB


  INTEGER,                         INTENT(IN), OPTIONAL :: NSOIL_GR !no. of green roof soil layers !aaron a.
  REAL,                            INTENT(INOUT), OPTIONAL :: DETENTION_STORAGE !Amount of water in detention pond of green roof [m] added by Aaron A.
  REAL,                            INTENT(IN), OPTIONAL :: FAREA !fractional area of the paired land types for lateral transfers
  ...
  !------Local variables for Green Roof - Aaron A.
  REAL :: PRECIPRATIO_GR
  REAL :: EXPRUN_GR
  REAL :: COEF_GR
  REAL :: STORAGEMAX_GR ! storage maximum in green roof [m]
  REAL :: DETENTIONSTORAGEMAX_GR ! detention storage in green roof [m]
  REAL :: ZSOIL_upper ! upper level soil to change back to normal (not green roofs)
...
GREEN_ROOF = .false.
  IF(VEGTYP.EQ.46) THEN
    IF (PRESENT(NSOIL_GR ) .AND. &
        PRESENT(DETENTION_STORAGE)) THEN
          PRECIPRATIO_GR = 0.65
          EXPRUN_GR = 1.67
          COEF_GR = 0.002*DT/60
          STORAGEMAX_GR = 0.07
          DETENTIONSTORAGEMAX_GR = 0.01
          GREEN_ROOF = .true.
    ELSE
      CALL wrf_error_fatal ('Green Roof module does not have values specified ')
    END IF
  END IF
...
! snow/soil layer thickness (m)
     ...
     IF (GREEN_ROOF) THEN
             DZSNSO(1) =   STORAGEMAX_GR/(parameters%SMCMAX(1))
             IF (ISNOW+1 == 1) THEN 
                ZSNSO(1) = -1* STORAGEMAX_GR/(parameters%SMCMAX(1))
             ELSE 
                ZSNSO(1) = ZSNSO(0) - STORAGEMAX_GR/(parameters%SMCMAX(1))
             END IF 
             ZSOIL_upper = ZSOIL(1)
             ZSOIL(1) = -1 * STORAGEMAX_GR/(parameters%SMCMAX(1))
    END IF
...
! total water storage for water balance check
     IF(IST == 1) THEN
     ! Green roof branching Aaron A.
       IF(GREEN_ROOF) THEN

        BEG_WB = CANLIQ + CANICE + SNEQV + WA + DETENTION_STORAGE*1000.
              
             DO IZ = 1,NSOIL_GR !Aaron A.
                 BEG_WB = BEG_WB + (SMC(IZ)+SMC_dummy(IZ)) * DZSNSO(IZ) * 1000.
             ENDDO
     ELSE ! no green roof

       BEG_WB = CANLIQ + CANICE + SNEQV + WA

       DO IZ = 1,NSOIL !Aaron A.
           BEG_WB = BEG_WB + (SMC(IZ)+SMC_dummy(IZ)) * DZSNSO(IZ) * 1000.
       ENDDO
...
!input GVF should be consistent with LAI
     ...
     IF(OPT_CROP > 0 .and. CROPTYPE > 0) THEN
        ...
        IF(FVEG <= 0.05) FVEG = 0.05
     ENDIF
     IF((parameters%urban_flag) .OR. (VEGTYP == parameters%ISBARREN ).or.(VEGTYP.eq.42).or.(VEGTYP.eq.44)) FVEG = 0.0
...
! compute energy budget (momentum & energy fluxes and phase changes)
    ...
    CALL ENERGY (parameters,ICE    ,VEGTYP ,IST    ,NSNOW  ,NSOIL  , & !in
		 ...
		 Z0WRF  ,                                         &
                 ...
                 TAUSS  ,LAISUN ,LAISHA ,RB , SMC_dummy, SH2O_dummy,                   & !inout !added by Aaron A. This will modify the call of thermal properties ONLY (variables named dummy)
!jref:start
                 QC     ,QSFC   ,PSFC   , & !in
                 T2MV   ,T2MB  ,FSRV   , &
...
!jref:end
                  GREEN_ROOF, NSOIL_GR) ! green roof specific variables Aaron A.
    ...
    EDIR = QVAP - QDEW
    BTRANI_dummy(1:NSOIL) = BTRANI(1:NSOIL) !Added by Aaron A.
...
! compute water budgets (water storages, ET components, and runoff)
     ...
     CALL WATER (parameters,VEGTYP ,NSNOW  ,NSOIL  ,IMELT  ,DT     ,UU     , & !in
		 ...
		 QSNOW  ,QRAIN  ,SNOWHIN,LATHEAV,LATHEAG,frozen_canopy,frozen_ground,  & !in  MB
                 ...
                 SMCWTD ,DEEPRECH,RECH, RUNONSRF, & !inout !ADDED by Aaron A.
                 CMC    ,ECAN   ,ETRAN  ,FWET   ,RUNSRF ,RUNSUB , & !out
                 QIN    ,QDIS   ,PONDING1       ,PONDING2,&
                 QSNBOT                             &
                ,GREEN_ROOF , NSOIL_GR, DETENTION_STORAGE, PRECIPRATIO_GR, EXPRUN_GR, & ! optional green roof values
                COEF_GR, STORAGEMAX_GR, DETENTIONSTORAGEMAX_GR, FAREA,DX, VOL_FLUX_SM, VOL_FLUX_RUNON )  !out
...
! water and energy balance check
     ...
     CALL ERROR (parameters,SWDOWN ,FSA    ,FSR    ,FIRA   ,FSH    ,FCEV   , & !in
                 ...
                 SNEQV  ,WA     ,SMC_dummy , SMC, DZSNSO ,PRCP   ,ECAN   , & !in !AARON A.
                 ETRAN  ,EDIR   ,RUNSRF ,RUNSUB ,DT     ,NSOIL  , & !in
                 NSNOW  ,IST    ,ERRWAT ,ILOC   , JLOC  ,FVEG   , &
                 SAV    ,SAG    ,FSRV   ,FSRG   ,ZWT    ,PAH    , &
                 PAHV   ,PAHG   ,PAHB,  RUNONSRF,VEGTYP, GREEN_ROOF, NSOIL_GR, DETENTION_STORAGE,FAREA   )   !in ( Except ERRWAT, which is out )

IF (VEGTYP.EQ.43) RUNONSRF = 0. !added by aaron A. Should zero out everyting we need
IF (VEGTYP.EQ.43) VOL_FLUX_RUNON = 0.
IF (VEGTYP.EQ.44) VOL_FLUX_RUNON = 0.
IF (VEGTYP.EQ.44) RUNONSRF = 0. !aaron A. Zero out runon so it doesnt blead over to other land use types
! urban - jref
    QFX = ETRAN + ECAN + EDIR
    IF (( parameters%urban_flag ) .OR.(VEGTYP.eq.41) .OR. (VEGTYP.eq.42) .OR. (VEGTYP.eq.44) .OR. (VEGTYP.eq.45)) THEN !modified by Aaron A.
...
IF (CROPTYPE == 0) THEN
  ...
  IF ( ( VEGTYP == parameters%iswater ) .OR. ( VEGTYP == parameters%ISBARREN ) .OR. &
       ( VEGTYP == parameters%ISICE   ) .or. ( parameters%urban_flag ) .OR. (VEGTYP.eq.42) .OR. (VEGTYP.eq.44) .OR. &
       ( VEGTYP.eq.45) ) THEN
     LAI  = 0.
     SAI  = 0.
  ENDIF
...
! initialization
      ...
      SNOWHIN = 0.0
      ICEDRIP = 0.0
!      print*, "precip_heat begin EAL, DIMENSION(MAX_SOIL_LEVELS) :: soil_thick_input
...
!== begin error ====================================================================================
  ...
  SUBROUTINE ERROR (parameters,SWDOWN ,FSA    ,FSR    ,FIRA   ,FSH    ,FCEV   , &
                    ...
                    SNEQV  ,WA     ,SMC2 ,SMC    ,DZSNSO ,PRCP   ,ECAN   , &
                    ETRAN  ,EDIR   ,RUNSRF ,RUNSUB ,DT     ,NSOIL  , &
                    NSNOW  ,IST    ,ERRWAT, ILOC   ,JLOC   ,FVEG   , &
                    SAV    ,SAG    ,FSRV   ,FSRG   ,ZWT    ,PAH    , &
                    PAHV   ,PAHG   ,PAHB, RUNONSRF, VEGTYP, GREEN_ROOF, NSOIL_GR, DETENTION_STORAGE,FAREA   )
! --------------------------------------------------------------------------------------------------
...
! inputs
  ...
  REAL                           , INTENT(IN) :: RUNONSRF
  INTEGER                        , INTENT(IN) :: VEGTYP ! added by Aaron A.

  LOGICAL                        , INTENT(IN), OPTIONAL :: GREEN_ROOF ! no. of soil layers in green roof  added by Aaron A.
  REAL                           , INTENT(IN), OPTIONAL :: DETENTION_STORAGE ! amount in the dentenion pond of green roof [m] added by Aaron A.
  ...
  REAL, DIMENSION(       1:NSOIL), INTENT(IN) :: SMC2    !soil moisture (ice + liq.) [m3/m3]
...
!   WRITE(*,*) "ERRENG =",ERRENG
   ...
   IF (IST == 1) THEN                                       !soil
        ...
        !This is the coupled tree/impermeable pavements
        IF (VEGTYP.eq.41) THEN

          END_WB = CANLIQ + CANICE + SNEQV + WA
          DO IZ = 1,NSOIL
            END_WB = END_WB + (SMC(IZ)+SMC2(IZ)) * DZSNSO(IZ) * 1000.
          END DO

          ERRWAT = END_WB-BEG_WB-(PRCP-ECAN-ETRAN-EDIR-RUNSRF-RUNONSRF-RUNSUB)*DT

          !This is an identical land-use type to an urban location, but
          !has been completely disconnected (meaning all water will be
          ! input to an adjacent turf grass or vegitatation type (43))
        ELSE IF( (VEGTYP.eq.42).or.(VEGTYP.eq.45) ) THEN
          END_WB = CANLIQ + CANICE + SNEQV + WA
          DO IZ = 1,NSOIL
            END_WB = END_WB + (SMC(IZ)+SMC2(IZ)) * DZSNSO(IZ) * 1000.
          END DO

          !ALL of the runoff is equal to runon for 42
          ERRWAT = END_WB-BEG_WB-(PRCP-RUNONSRF-ECAN-ETRAN-EDIR-RUNSUB)*DT


          !this is the recipee for eveything else!

        ELSE IF (GREEN_ROOF) THEN
          END_WB = CANLIQ + CANICE + SNEQV + WA + DETENTION_STORAGE * 1000.
          
          DO IZ = 1,NSOIL_GR
            END_WB = END_WB + (SMC(IZ)+SMC2(IZ)) * DZSNSO(IZ) * 1000.
          END DO
          ERRWAT = END_WB-BEG_WB-(PRCP+RUNONSRF-ECAN-ETRAN-EDIR-RUNSRF-RUNSUB)*DT
        ...
        IF(ABS(ERRWAT) > 0.1) THEN
           ...
           call wrf_message(trim(message))
           WRITE(message, &
           '("    I      J     END_WB     BEG_WB       PRCP       ECAN       EDIR      ETRAN      RUNSRF     RUNSUB   ZWT   RUNONSRF  DETENTION_STORAGE")')
           call wrf_message(trim(message))
           WRITE(message,'(i6,1x,i6,1x,2f15.3,9f11.5)')ILOC,JLOC,END_WB,BEG_WB,PRCP*DT,ECAN*DT,&
                EDIR*DT,ETRAN*DT,RUNSRF*DT,RUNSUB*DT,ZWT, RUNONSRF*DT, DETENTION_STORAGE*1000.
           call wrf_message(trim(message))
...
!== begin energy ===================================================================================
  ...
  SUBROUTINE ENERGY (parameters,ICE    ,VEGTYP ,IST    ,NSNOW  ,NSOIL  , & !in
		     ...
		     Z0WRF  ,                                        &
                     ...
                     SNEQVO ,SNEQV  ,SH2O   ,SMC    ,SNICE  ,SNLIQ  , & !inout
                     ALBOLD ,CM     ,CH     ,DX     ,DZ8W   ,Q2     , &   !inout
                     TAUSS  ,LAISUN ,LAISHA ,RB ,SMC_dummy, SH2O_dummy,                     & !inout !added by Aaron A. This is to modify the thermal properties of vegetation grid
!jref:start
                     QC     ,QSFC   ,PSFC   , & !in
                     T2MV   ,T2MB   ,FSRV   , &
                     FSRG   ,RSSUN  ,RSSHA  ,ALBSND  ,ALBSNI,BGAP   ,WGAP,TGV,TGB,&
...
!  REAL                                              :: Q2E
  ...
  REAL   , DIMENSION(       1:NSOIL), INTENT(INOUT) :: SH2O_dummy   !liquid soil moisture [m3/m3] !SHOULD ONLY BE PASSED THE THERMPROPS
  REAL   , DIMENSION(       1:NSOIL), INTENT(INOUT) :: SMC_dummy    !soil moisture (ice + liq.) [m3/m3] ! SHOULD ONELY BE PASSED TO THERMPROPS
...
! Green Roof Variables needed for Energy Call
LOGICAL, INTENT(IN) :: GREEN_ROOF
INTEGER, INTENT(IN) :: NSOIL_GR
...
! special case for urban
     ...
     IF ((parameters%urban_flag).OR.(VEGTYP.eq.42).OR.(VEGTYP.eq.44).OR.(VEGTYP.eq.45) )THEN !Added by Aaron A.
     ...
     !IF(UR < 1.5) UR = 1.5
     !IF(UR < 1.5) Z0MG = 10*Z0MG
     !IF(UR < 1.5) Z0M = 10*Z0M
! canopy wind absorption coefficient
...
! Thermal properties of soil, snow, lake, and frozen soil
  ...
  CALL THERMOPROP (parameters,NSOIL   , NSNOW   ,ISNOW   ,IST     ,DZSNSO  , & !in
                   ...
                   SMC     ,SH2O    ,TG      ,STC     ,UR      , & !in !modified by Aaron A. This now should make the heat capacity of the bulk urban area different. Increasing heating
                   LAT     ,Z0M     ,ZLVL    ,VEGTYP  ,  & !in
                   DF      ,HCPCT   ,SNICEV  ,SNLIQV  ,EPORE   , & !out
                   FACT    , GREEN_ROOF, NSOIL_GR )   !out and Green Roof Added by Aaron A.
...
! ground emissivity is modified by G. Aaron Alexander
! This has been modified to reflect the changes in urban emissivity and
     EMV = 1. - EXP(-(ELAI+ESAI)/1.0)
     IF (ICE == 1) THEN
       EMG = 0.98*(1.-FSNO) + 1.0*FSNO
  !   ELSE IF((parameters%urban_flag)) THEN 
  !     EMG = 0.94*(1.-FSNO) + 1.0*FSNO
  !   ELSE IF((VEGTYP.eq.42).OR.(VEGTYP.eq.44).OR.(VEGTYP.eq.45).OR.(VEGTYP.EQ.41)) THEN 
  !     EMG = 0.85*(1.-FSNO) + 1.0*FSNO
     ELSE
     ...
     IF(IST ==1 ) THEN
       DO IZ = 1, parameters%NROOT
          IF(OPT_BTR == 1) then                  ! Noah
            GX    = (SH2O_dummy(IZ)-parameters%SMCWLT(IZ)) / (parameters%SMCREF(IZ)-parameters%SMCWLT(IZ))
          END IF
          IF(OPT_BTR == 2) then                  ! CLM
            PSI   = MAX(PSIWLT,-parameters%PSISAT(IZ)*(MAX(0.01,SH2O_dummy(IZ))/parameters%SMCMAX(IZ))**(-parameters%BEXP(IZ)) )
...
! soil surface resistance for ground evap.
     ...
     ELSE
       ...
       PSI   = -parameters%PSISAT(1)*(MAX(0.01,SH2O_dummy(1))/parameters%SMCMAX(1))**(-parameters%BEXP(1))
...
! 3L snow & 4L soil temperatures
    ...
    CALL TSNOSOI (parameters,ICE     ,NSOIL   ,NSNOW   ,ISNOW   ,IST     , & !in
                  ...
                  TG      ,ILOC    ,JLOC    ,                   & !in
                  STC, GREEN_ROOF, NSOIL_GR  )              !inout Changed by Aaron A.
...
! Energy released or consumed by snow & frozen soil
 ...
 CALL PHASECHANGE (parameters,NSNOW   ,NSOIL   ,ISNOW   ,DT      ,FACT    , & !in
                   ...
                   SMC     ,SH2O    ,                            & !inout
                   QMELT   ,IMELT   ,PONDING, GREEN_ROOF, NSOIL_GR )                     !out
...
! inputs
  ...
  LOGICAL                        , INTENT(IN), OPTIONAL  :: GREEN_ROOF !logical to get to green roof physics
  INTEGER                        , INTENT(IN), OPTIONAL  :: NSOIL_GR !no. of soil layers in green roof
! outputs
...
IF (GREEN_ROOF) THEN !add green roof (only a single layer)
    DO  IZ = 1, NSOIL
      IF (IZ.eq.NSOIL_GR) THEN
       SICE(IZ)  = SMC(IZ) - SH2O(IZ)
       HCPCT(IZ) = SH2O(IZ)*CWAT + (1.0-parameters%SMCMAX(IZ))*1.125*1000000 &
...
ELSE
    ...
    END DO
END IF
IF (( parameters%urban_flag ).OR.(VEGTYP.EQ.41).OR.(VEGTYP.EQ.42).OR.(VEGTYP.EQ.44).OR.(VEGTYP.EQ.45)) THEN !added by Aaron A.
       DO IZ = 1,NSOIL
         DF(IZ) = 3.24
       END DO
    ENDIF
!    IF (( parameters%urban_flag ))THEN !added by Aaron A.
!       DO IZ = 1,NSOIL
!         DF(IZ) = 3.24 ! adjusted by Aaron A. 
!         HCPCT(IZ) = 1.8125*1000000
!       END DO
!    ENDIF

!    IF ((VEGTYP.EQ.42))THEN !added by Aaron A.
!       DO IZ = 1,NSOIL
!         DF(IZ) = 3.24 ! adjusted by Aaron A. 
!         HCPCT(IZ) = 1.685*1000000
!       END DO
!    ENDIF

! change values for asphalt     
 !   IF( (VEGTYP.EQ.41) .OR. (VEGTYP.EQ.45)  ) THEN 
 !     DO IZ = 1,NSOIL 
 !       DF(IZ) = 3.24
 !       HCPCT(IZ) = 1.94*1000000
 !     END DO
 !   END IF 
! adjust the conducitivyt of the first level to be aslphalt for permeable pavement 
 !  IF ( (VEGTYP.EQ.44) ) THEN
 !    DF(1) = 3.24
 !    HCPCT(1) = 1.94*1000000
 !  END IF 
...
IF (GREEN_ROOF) THEN ! added by Aaron A. Green roof
  DO IZ = ISNOW+1,NSOIL_GR
...
! --------------------------------------------------------------------------------------------------
  ...
  DO IB = 1, NBAND
        ...
        IF (IST .EQ. 1)  THEN                     !soil
           ...
           ALBSOI = ALBSOD
   !    ELSE IF((IST .EQ. 1).AND.(parameters%urban_flag)) THEN !aaron A. 
   !     IF (IB.EQ.1) THEN 
   !        ALBSOD = 0.14
   !        ALBSOI = 0.14
   !     ELSE 
   ...
   !     END IF 
   !     ! Aaron A.         
   !     ELSE IF((IST.EQ.1).AND.((VEGTYP.EQ.41).OR.(VEGTYP.EQ.42).OR.(VEGTYP.EQ.45).OR.(VEGTYP.EQ.44))) THEN
   !     IF (IB.EQ.1) THEN
   !        ALBSOD = 0.1
   !        ALBSOI = 0.1
   !     ELSE
...
!== begin tsnosoi ==================================================================================
  ...
  SUBROUTINE TSNOSOI (parameters,ICE     ,NSOIL   ,NSNOW   ,ISNOW   ,IST     , & !in
                      ...
                      SAG     ,DT      ,SNOWH   ,DZSNSO  , & !in
                      TG      ,ILOC    ,JLOC    ,                   & !in
                      STC, GREEN_ROOF, NSOIL_GR )                   !inout & Green Roof adjusted by Aaron A.
! --------------------------------------------------------------------------------------------------
...
! Green Roof
   LOGICAL, INTENT(IN), OPTIONAL :: GREEN_ROOF ! Logical for green roof
   INTEGER, INTENT(IN), OPTIONAL :: NSOIL_GR ! no. soil layers (1)
!local
...
! compute soil temperatures
      ...
      CALL HRT   (parameters,NSNOW     ,NSOIL     ,ISNOW     ,ZSNSO     , &
                  ...
                  DF        ,HCPCT     ,SSOIL     ,PHI       , &
                  AI        ,BI        ,CI        ,RHSTS     , &
                  EFLXB     ,GREEN_ROOF, NSOIL_GR)

      CALL HSTEP (parameters,NSNOW     ,NSOIL     ,ISNOW     ,DT        , &
                  AI        ,BI        ,CI        ,RHSTS     , &
                  STC       ,GREEN_ROOF, NSOIL_GR  )
...
! Optional Green Roof
   LOGICAL, INTENT(IN), OPTIONAL :: GREEN_ROOF ! aaron A.
   INTEGER, INTENT(IN), OPTIONAL :: NSOIL_GR ! aaron A.
! local
    ...
    REAL                                         :: TEMP1
    INTEGER                                      :: NSOIL_ACT ! local variable for number of soil layers,
                                                              ! allows for swapping between Green roof and non-green roof
                                                              ! without re-writing
! ----------------------------------------------------------------------
    ...
    ELSE
        NSOIL_ACT = NSOIL
    END IF
!normal code (NSOIL swapped with NSOIL_ACT ) Aaron A.
    DO K = ISNOW+1, NSOIL_ACT
        IF (K == ISNOW+1) THEN
...
! solve the tri-diagonal matrix equation
    ...
    CALL ROSR12 (CI,AI,BI,CIIN,RHSTSIN,RHSTS,ISNOW+1,NSOIL,NSNOW,GREEN_ROOF,NSOIL_GR)
...
!== begin rosr12 ===================================================================================
  ...
  SUBROUTINE ROSR12 (P,A,B,C,D,DELTA,NTOP,NSOIL,NSNOW,GREEN_ROOF,NSOIL_GR)
! ----------------------------------------------------------------------
! SUBROUTINE ROSR12
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
    P (NSOIL_ACT) = DELTA (NSOIL_ACT)
! ----------------------------------------------------------------------
! ADJUST P FOR SOIL LAYERS 2 THRU NSOIL
! ----------------------------------------------------------------------
    DO K = NTOP+1,NSOIL_ACT
       KK = NSOIL_ACT - K + (NTOP-1) + 1
...
! local
  ...
  ! add quick switch for Green roof without messing with variable dimensions Aaron A.
  IF (GREEN_ROOF) THEN
      NSOIL_ACT = NSOIL_GR
  ELSE
      NSOIL_ACT = NSOIL
...
!== begin water ====================================================================================
  ...
  SUBROUTINE WATER (parameters,VEGTYP ,NSNOW  ,NSOIL  ,IMELT  ,DT     ,UU     , & !in
		    ...
		    QSNOW  ,QRAIN  ,SNOWHIN,LATHEAV,LATHEAG,frozen_canopy,frozen_ground,    & !in  MB
                    ...
                    SNICE  ,SNLIQ  ,STC    ,ZSNSO  ,SH2O   ,SMC    , & !inout
                    SICE   ,ZWT    ,WA     ,WT     ,DZSNSO ,WSLAKE , & !inout
                    SMCWTD ,DEEPRECH,RECH,  RUNONSRF,                & !inout
                    CMC    ,ECAN   ,ETRAN  ,FWET   ,RUNSRF ,RUNSUB , & !out
                    QIN    ,QDIS   ,PONDING1       ,PONDING2,        &
                    QSNBOT                                           &
...
! local
  ...
  REAL, DIMENSION(       1:NSOIL)                :: ETRANI_OLD !added by Aaron A.
  REAL, PARAMETER ::  WSLMAX = 5000.      !maximum lake water storage (mm)

 REAL :: NSOIL_ACT
 REAL, INTENT(IN), OPTIONAL          :: PRECIPRATIO_GR
 REAL, INTENT(IN), OPTIONAL          :: EXPRUN_GR
 REAL, INTENT(IN), OPTIONAL          :: COEF_GR
 REAL, INTENT(IN), OPTIONAL          :: STORAGEMAX_GR ! storage maximum in green roof [m]
 REAL, INTENT(IN), OPTIONAL          :: DETENTIONSTORAGEMAX_GR ! detention storage in green roof [m]
 REAL, INTENT(INOUT), OPTIONAL       :: DETENTION_STORAGE
 LOGICAL, INTENT(IN), OPTIONAL       :: GREEN_ROOF
 INTEGER, INTENT(IN), OPTIONAL       :: NSOIL_GR
 
  REAL,                            INTENT(IN), OPTIONAL :: FAREA !fractional area of the paired land types for lateral transfers
  REAL, DIMENSION(1:NSOIL),        INTENT(INOUT), OPTIONAL :: VOL_FLUX_SM !volume flux of soil (per layer) pulled from tree sharing soil moisture in Urban Environment Aaron A. 
  REAL,                            INTENT(INOUT), OPTIONAL :: VOL_FLUX_RUNON !volumen flux of run-on from one grid to the another grid (for paired land types) Aaron A. 
  REAL,                            INTENT(IN), OPTIONAL :: DX !spacing of the model grid (needed for volumes)
!----------
! add quick switch for Green roof without messing with variable dimesnions Aaron A.
IF (GREEN_ROOF) THEN
    NSOIL_ACT = NSOIL_GR
...
IF( (VEGTYP.EQ.43).OR.(VEGTYP.EQ.44)) THEN
  RUNONSRF = (VOL_FLUX_RUNON/(FAREA*DX*DX*DT)) * 1000 !runon is now a rate of mm/s
  !RUNONSRF = RUNONSRF * FAREA
ELSE
  RUNONSRF = 0.
END IF

    IF(ISNOW == 0) THEN
       WRITE(*,*) 'ILOC', ILOC, 'JLOC', JLOC,'RUNONSRF:', RUNONSRF, 'VOL_FLUX_RUNON',VOL_FLUX_RUNON, 'FAREA', FAREA
    

      IF ((VEGTYP.EQ.44).OR.(VEGTYP.EQ.43) )THEN
    ...
    DO IZ = 1, parameters%NROOT !Aaron A.
       ETRANI(IZ) = ETRAN * BTRANI(IZ) * 0.001 + ETRANI(IZ)
       ETRANI_OLD(IZ) = ETRANI(IZ)
       VOL_FLUX_SM(IZ) = ETRANI_OLD(IZ)*FAREA*DX*DX*DT
...
! lake/soil water balances
    ...
    ELSE                                                      ! soil
       CALL      SOILWATER (parameters,NSOIL  ,NSNOW  ,DT     ,ZSOIL  ,DZSNSO , & !in
                            QINSUR ,QSEVA  ,ETRANI ,SICE   ,ILOC   , JLOC , CANLIQ, & !in
                            SH2O   ,SMC   ,ZWT    ,VEGTYP , & !inout AARON A.
                           SMCWTD, DEEPRECH, RUNONSRF                       , & !inout !changed by aaron A.
                            RUNSRF ,QDRAIN ,RUNSUB ,WCND   ,FCRMAX, & !additions by Aaron A.
                            GREEN_ROOF, NSOIL_GR, DETENTION_STORAGE, PRECIPRATIO_GR, EXPRUN_GR, & ! Optional Green Roof
                            COEF_GR, STORAGEMAX_GR, DETENTIONSTORAGEMAX_GR)   !out

       IF((OPT_RUN == 1).and.(GREEN_ROOF == .false.)) THEN ! Aaron A.
       ...
       IF((OPT_RUN == 5).and.(GREEN_ROOF == .false.)) THEN ! Aaron A.
...
!very last step: convert the runon to be a volume for later use: 
   VOL_FLUX_RUNON = RUNONSRF * 0.001 *FAREA* DX*DX*DT !now is cubic meters
   WRITE(*,*) 'ILOC', ILOC, 'JLOC', JLOC,'RUNONSRF:', RUNONSRF, 'VOL_FLUX_RUNON',VOL_FLUX_RUNON, 'FAREA', FAREA
...
! input & output
  ...
  REAL,                     INTENT(INOUT) :: RUNONSRF !added by Aaron A.
...
! local
  ...
  REAL, INTENT(IN), OPTIONAL          :: STORAGEMAX_GR ! storage maximum in green roof [m]
  REAL, INTENT(IN), OPTIONAL          :: DETENTIONSTORAGEMAX_GR ! detention storage in green roof [m]
  REAL, INTENT(INOUT), OPTIONAL       :: DETENTION_STORAGE ! depth of water stored in the denetenion pond of green roof [m]
  LOGICAL, INTENT(IN), OPTIONAL       :: GREEN_ROOF
  INTEGER, INTENT(IN), OPTIONAL       :: NSOIL_GR
...
! ----------------------------------------------------------------------
    ...
    IF ((VEGTYP.EQ.41).OR.(VEGTYP.EQ.42).OR.(VEGTYP.EQ.45)) THEN
      RUNONSRF = 0.0 !added by Aaron A.
    ENDIF
...
! Green Roof Branching If Else statement. If green roof do simple calculations,
! if not, do the typical calculations

IF (GREEN_ROOF) THEN
      CALL SOILWATER_greenroof(parameters,NSNOW,NSOIL, QINSUR, QSEVA, ETRANI, DT, STORAGEMAX_GR, FCR,&
                                    PRECIPRATIO_GR, COEF_GR, EXPRUN_GR, DETENTIONSTORAGEMAX_GR, &
                                    SH2O,SICE, ZSOIL, DETENTION_STORAGE, RUNSRF)
...
!jref impermable surface at urban
    IF (( parameters%urban_flag).OR.(VEGTYP.EQ.41).OR.(VEGTYP.EQ.42).OR.(VEGTYP.EQ.45)) THEN
    FCR(1)= .99
    ENDIF
...
! solve soil moisture
    ...
    ! runon partitioning and now in mm/s
...
END IF ! Green Roof Branch Aaron A.

   IF (VEGTYP.EQ.41) THEN !the runon partitioning between run on and run off. Currently set up to be only runoff
    RUNONSRF = 0.
    RUNSRF = RUNSRF
   END IF

   IF (VEGTYP.EQ.42) THEN !this controls the partitioning between runon and runoff for urban dis-connected
    RUNONSRF = RUNSRF
    RUNSRF = 0.
   END IF
   
   IF (VEGTYP.EQ.45) THEN !this controls the partitioning between runon and runoff for permeable pavements
    RUNONSRF = RUNSRF
    RUNSRF = 0.
   END IF
...
! inputs
  ...
  INTEGER,                  INTENT(IN) :: IVGTYP !Vegitation type ! Added by Aaron A.
...
! jref for urban areas
      IF ( parameters%urban_flag ) INFMAX = INFMAX * 0.00 !added by aaron
      IF ( IVGTYP.EQ.42) INFMAX = INFMAX * 0.00 !added by aaron A. (Urban runoff to turfgrass)
      IF ( IVGTYP.EQ.41) INFMAX = INFMAX * 0.00 !added by Aaron A (Urban Tree Cover)
      IF ( IVGTYP.EQ.45) INFMAX = INFMAX * 0.00 ! added by Aaron A. (Urban with runoff to permeable pavement)      
...
! hydraulic conductivity
  ...
  !== begin SOILWATER_greenroof ==============================================================================

  !  subroutine SOILWATER_greenroof
  !  ! BASED ON LOCATELLI ET AL 2014. INSTEAD OF USING ITERATIONS ON THE
  !  ! RICHARDS EQUATION, WHICH IS NOT NECESSARY FOR GREEN ROOFS DUE TO THEIR
  !  ! SOIL PROPERTIES, WE HAVE IMPLEMENTED A SIMPLER THREE BUCKET MODEL. ONE
  !  ! FOR VEGETATION, ONE FOR SOIL WATER HOLDING, AND ONE FOR OUTFLOW.
  !===============================================================================

        SUBROUTINE SOILWATER_greenroof(parameters,NSOIL, NSNOW, QINSUR, QSEVA, ETRANI, DT, STORAGEMAX, FCR, &
                                      PRECIPRATIO_GR, COEF_GR, EXPRUN_GR, DETENTION_STORAGE_MAX, &
                                      SH2O,SICE, ZSOIL,DETENTION_STORAGE, RUNOFF)

        IMPLICIT NONE

  !-------------------------------------------------------------------------------
  ! I: input variables from from main driver
  !-------------------------------------------------------------------------------
        type (noahmp_parameters), INTENT(IN)   :: parameters

        INTEGER,                   INTENT(IN)      :: NSOIL        ! no. of soil layers (4)
        INTEGER,                         INTENT(IN) :: NSNOW  !maximum no. of snow layers
        REAL,                      INTENT(IN)      :: QINSUR       ! INPUT PRECIPITATION [M/S]
        REAL,                      INTENT(IN)      :: QSEVA        ! evap from soil surface [m/s]
        REAL, DIMENSION(1:NSOIL), INTENT(IN)       :: ETRANI       ! evapotranspriation from soil layers [mm/s]
        REAL,                      INTENT(IN)      :: DT           ! TIMESTEP [S]
        REAL,                      INTENT(IN)      :: STORAGEMAX   ! MAXIMUM DEPTH ALLOWED IN THE SOIL MEDIUM [M]
        REAL, DIMENSION(1:NSOIL),  INTENT(IN)      :: FCR
        REAL,                      INTENT(IN)      :: PRECIPRATIO_GR            ! FRACTIONAL THRESHOLD AT WHICH WATER DRAINS
                                                           ! FROM SOIL MEDIUM TO RENTION POND WHILE RAINING [-]
        REAL,                      INTENT(IN)      :: COEF_GR                   ! NONLINEAR OUTFLOW COEFFICIENT [M^(1-N) S^-1]
        REAL,                      INTENT(IN)      :: EXPRUN_GR                 ! NONLINEAR OUTFLOW EXPONENT [-]
        REAL,                      INTENT(IN)      :: DETENTION_STORAGE_MAX     ! MAXIMUM AMOUNT OF WATER HELD IN
                                                           ! DETENTION STORAGE BEFORE GREEN ROOF IS OVERWHELMED
        REAL, DIMENSION(1:NSOIL),    INTENT(IN) :: ZSOIL  !depth of soil layer-bottom [m]

        REAL, DIMENSION(1:NSOIL), INTENT(INOUT)   :: SH2O                       ! VOLUMETRIC SOIL MOISTURE [M^3 M^-3]
        REAL, DIMENSION(1:NSOIL), INTENT(IN)   :: SICE                       ! ICE SOIL CONTENT
        REAL,                     INTENT(INOUT)   :: DETENTION_STORAGE         ! AMOUNT OF WATER STORED GREENROOF STORAGE POND [M]
        REAL,                     INTENT(INOUT)   :: RUNOFF       ! AMOUNT OF WATER OUT OF THE DETENTION POND [M/S]
  ...
  !-------------------------------------------------------------------------------
        ...
        REAL                 :: STORAGE ! DEPTH OF THE SOIL WATER [M]
        REAL                 :: OUTFLOW_ONE ! RATE OF OUTFLOW FROM SOIL MEDIUM TO DETENTION POND ! [M/S]
        REAL                 :: STORAGE_NEW ! NEW DEPTH OF SOIL WATER [M]
        REAL                 :: DETENTION_STORAGE_NEW
        REAL                 :: QINSUR_MOD
        REAL                 :: RUNOFF_FRZ
        REAL                 :: RUNOFF_ADDITION
        REAL                 :: STORAGE_ICE
  !-------------------------------------------------------------------------------
  ! LINEARLY MAP FROM SOIL MOISTURE TO STORAGE BASED ON SOIL PROPERTIES
  !-------------------------------------------------------------------------------
        !STORAGE = ((STORAGEMAX)/(parameters%SMCMAX(1) - parameters%SMCDRY(1))) * (SH2O(1) - parameters%SMCDRY(1))
         STORAGE = SH2O(1) * ZSOIL(1)*-1
         STORAGE_ICE = SICE(1) * ZSOIL(1)*-1
  !-------------------------------------------------------------------------------
  ! CALCULATE THE INFILTRATION. BASED ON LOCATELLI ET AL 2014
  ! https://doi.org/10.1016/j.jhydrol.2014.10.030
  !-------------------------------------------------------------------------------
  ! adjust the incoming water based on the frozen state of the water.
      IF (FCR(1).gt.0.0) THEN
          RUNOFF_FRZ = QINSUR*FCR(1)
          QINSUR_MOD = QINSUR*(1-FCR(1))
      ELSE
          QINSUR_MOD = QINSUR
          RUNOFF_FRZ = 0.
     END IF 
  !-------------------------------------------------------------------------------
  ! BOX ONE: SOIL MOISTURE STORAGE AREA
  !-------------------------------------------------------------------------------
          IF ( ((STORAGE+STORAGE_ICE)/STORAGEMAX).GT.PRECIPRATIO_GR) THEN
            IF (QINSUR_MOD.GT.0) THEN

              OUTFLOW_ONE = QINSUR_MOD * (((STORAGE+STORAGE_ICE)/STORAGEMAX) - PRECIPRATIO_GR)/(1-PRECIPRATIO_GR)
          ...
          STORAGE_NEW = MAX(0.0, STORAGE + QINSUR_MOD*DT - OUTFLOW_ONE*DT -ETRANI(1)*DT - QSEVA*DT)
          WRITE(*,*) 'STORAGEMAX:', STORAGEMAX, 'STORAGE NEW', STORAGE_NEW
  !-------------------------------------------------------------------------------
  ! If there is too much moisture, in the green roof, create the an overland flow 
  ! flow addition
  !-------------------------------------------------------------------------------
         IF (STORAGE_NEW >= STORAGEMAX) THEN
            WRITE(*,*) 'ENGAGE OVERLAND FLOW AT SOIL SURFACE'
            RUNOFF_ADDITION = STORAGE_NEW - STORAGEMAX
            STORAGE_NEW = STORAGEMAX
         ELSE
            RUNOFF_ADDITION = 0.
         ENDIF
  !-------------------------------------------------------------------------------
  ! BOX TWO: RETENTION POND. IF WATER HAS ENTERED INTO THE RETENTION POND, IT WILL
  ! BE RUNOFF, BUT USING A NON-LINEAR MODEL
  !-------------------------------------------------------------------------------
          RUNOFF = COEF_GR*(DETENTION_STORAGE**EXPRUN_GR)

  !-------------------------------------------------------------------------------
  ! CORRECT IF WE ARE REMOVING MORE WATER THAN IS IN DETENTION STORAGE
  !-------------------------------------------------------------------------------

          IF ((RUNOFF*DT).GT.DETENTION_STORAGE) THEN
            RUNOFF = DETENTION_STORAGE/DT
            WRITE(*,*) 'ENGAGED OVERFLOW DETENTION'        
ENDIF
  !-------------------------------------------------------------------------------
  ! CREATE NEW DETENTION STORAGE (INPUTS + CURRENT - OUTFLOW)
  !-------------------------------------------------------------------------------
          DETENTION_STORAGE_NEW = OUTFLOW_ONE*DT + DETENTION_STORAGE - RUNOFF*DT

  !-------------------------------------------------------------------------------
  ! MODIFY THE RUNOFF AND STORAGE DEPENDING ON LIMITS OF THE STORAGE POND
  !-------------------------------------------------------------------------------
          IF (DETENTION_STORAGE_NEW.GT.DETENTION_STORAGE_MAX) THEN
            RUNOFF = RUNOFF + (DETENTION_STORAGE_NEW - DETENTION_STORAGE_MAX)/DT
            DETENTION_STORAGE_NEW = DETENTION_STORAGE_MAX
            WRITE(*,*) 'ENGAGED OVERLAND FLOW CORRECTION'
          ENDIF
  !-------------------------------------------------------------------------------
  ! FINALLY CHECK IF THE NEW STORAGE IS EQUAL LESS THAN ZERO AND ADJUST
  !-------------------------------------------------------------------------------

          IF (DETENTION_STORAGE_NEW.LT.0) THEN
            DETENTION_STORAGE_NEW = 0.
          ENDIF

  !-------------------------------------------------------------------------------
  ! FINALY RE-MAP FROM STORAGE TO SOIL MOISTURE:
  !-------------------------------------------------------------------------------
           SH2O(1) = STORAGE_NEW/(-1*ZSOIL(1))
          !SH2O(1) = ((parameters%SMCMAX(1) - parameters%SMCDRY(1))/(STORAGEMAX))*STORAGE_NEW + parameters%SMCDRY(1)
         WRITE(*,*) 'SH2O',SH2O(1), 'ZSOIL',ZSOIL
         DETENTION_STORAGE = DETENTION_STORAGE_NEW
          RUNOFF = (RUNOFF_FRZ + RUNOFF + RUNOFF_ADDITION/DT )*1000 ! now in mm/s
  !---------------------------------------------------
          END SUBROUTINE SOILWATER_greenroof
  !---------------------------------------------------
...
! ------------------------------------------------------------------------------------------
   ...
   IF ( ( VEGTYP == parameters%iswater ) .OR. ( VEGTYP == parameters%ISBARREN ) .OR. &
        ( VEGTYP == parameters%ISICE ) .or. (parameters%urban_flag) .or.(VEGTYP.eq.42) .or. (VEGTYP.eq.44) ) THEN ! adjusted by Aaron A.
      XLAI   = 0.
      XSAI   = 0.
      GPP    = 0.
      NPP    = 0.
...
! ------------------------------------------------------------------------------------------
   IF ( ( VEGTYP == parameters%iswater ) .OR. ( VEGTYP == parameters%ISBARREN ) .OR. &
        ( VEGTYP == parameters%ISICE ) .or. (parameters%urban_flag) .or. (VEGTYP.eq.42) .or. (VEGTYP.eq.44) ) THEN ! adjsuted by Aaron A.
      XLAI   = 0.
      XSAI   = 0.
      GPP    = 0.
      NPP    = 0.
...
MODULE NOAHMP_TABLES
    ...
    INTEGER, PRIVATE, PARAMETER :: MVT   = 47 !modified by Aaron A. 16 JUN 2021 NLCD
    INTEGER, PRIVATE, PARAMETER :: MBAND = 2
