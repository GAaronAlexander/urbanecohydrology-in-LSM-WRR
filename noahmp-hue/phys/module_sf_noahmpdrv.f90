  ...
  SUBROUTINE noahmplsm(ITIMESTEP,        YR,   JULIAN,   COSZIN,XLAT,XLONG, & ! IN : Time/Space-related
		...
		ACSNOW,    EMISS,     QSFC,                                 & ! IN/OUT LSM eqv
	       ...
	       SMOISEQ, SMCWTDXY,DEEPRECHXY,   RECHXY,  GRAINXY,    GDDXY,PGSXY,  & ! IN/OUT Noah MP only
	        ...
	        TRADXY,    NEEXY,    GPPXY,     NPPXY,   FVEGXY,   RUNSFXY,  RUNONSFXY,& ! OUT Noah MP only !added by Aaron A.
		...
		BGAPXY,   WGAPXY,    TGVXY,     TGBXY,    CHVXY,     CHBXY, & ! OUT Noah MP only
		 ...
		 GHBXY,    IRGXY,    IRCXY,     IRBXY,     TRXY,     EVCXY, & ! OUT Noah MP only
              CHLEAFXY,   CHUCXY,   CHV2XY,    CHB2XY, RS, BTRANI_output,   & ! OUT Noah MP only
...
! OUT (with no Noah LSM equivalent)
    ...
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  RUNONSFXY ! Surface runon Added by Aaron A.
    ...
    REAL,    DIMENSION( ims:ime, 1:nsoil, jms:jme ), INTENT(INOUT) :: BTRANI_output !added by Aaron A.
    !---------------------
    !Green Roof NOAH-MP Additions Aaron A.
    !---------------------
    REAL,    DIMENSION( ims:ime, 1:NSOIL, jms:jme ), INTENT(INOUT), OPTIONAL :: TSLB_PER_PAVEMENT    ! Permeable Pavement soil temperature [k]
    REAL,    DIMENSION( ims:ime, 1:NSOIL, jms:jme ), INTENT(INOUT), OPTIONAL :: SMOIS_PER_PAVEMENT   ! Permeable Pavement soil moisture [m3/m3]
    REAL,    DIMENSION( ims:ime, 1:NSOIL, jms:jme ), INTENT(INOUT), OPTIONAL :: SH2O_PER_PAVEMENT    ! Permeable Pavement liquid water content [m3/m3]
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(INOUT), OPTIONAL :: DETENTION_STORAGEXY  ! detention storage
    
    !---------------------
    !lateral transfers Additions Aaron A.
    !---------------------
    REAL,   DIMENSION( ims:ime,          jms:jme),   INTENT(IN), OPTIONAL :: FAREAXY !fractional area (does not change)
    REAL,   DIMENSION( ims:ime, 1:NSOIL, jms:jme),  INTENT(INOUT), OPTIONAL :: VOL_FLUX_SMXY !fractional soil moisture volume pulled from sharing 
    REAL,   DIMENSION( ims:ime         , jms:jme),  INTENT(INOUT), OPTIONAL :: VOL_FLUX_RUNONXY
...
! OUT (with no Noah LSM equivalent)
    ...
    REAL                                :: RUNONSF      ! surface runon [mm/s]
...
! Intermediate terms
    ...
    !create intermediate soil moisture terms Aaron A.
...
! IN/OUT
! Green NOAH - MP changes for inputs
      IF (VEGTYP.EQ.41) THEN !Added by Aaron Alexander
        ...
        SMC_intermediate (1:NSOIL) = SMOIS(I+1,1:NSOIL,J)  !  This is the soil moisture of the fully vegetated square
        SH2O_intermediate(1:NSOIL) = SH2O(I+1,1:NSOIL,J)   !  This is the soil moisture of the fully vegetated square

      ELSE IF (VEGTYP.EQ.44) THEN ! This is to ensure that the soil moisture that is being used is only for the Permeable Pavements

        SMC  (      1:NSOIL)  = SMOIS_PER_PAVEMENT  (I,      1:NSOIL,J)  ! soil total moisture [m3/m3]
        SMH2O(      1:NSOIL)  = SH2O_PER_PAVEMENT   (I,      1:NSOIL,J)  ! soil liquid moisture [m3/m3]
        
        SMC_intermediate (1:NSOIL) = SMOIS_PER_PAVEMENT(I,1:NSOIL,J)
        SH2O_intermediate(1:NSOIL) = SH2O_PER_PAVEMENT(I,1:NSOIL,J)
      ...
      ! GREEN ROOF
      DETENTION_STORAGE = DETENTION_STORAGEXY(I,J)

      ! add a specific if else switch for soil temperatures

      IF (VEGTYP.EQ.44) THEN !Different temperature values for permeable pavements
        STC(1:NSOIL) = TSLB_PER_PAVEMENT(I, 1:NSOIL, J) !  permeable pavement temperature [K]
      ELSE

        STC(1:NSOIL) = TSLB (I, 1:NSOIL, J) ! Soil Temperatures [K]
      ENDIF
! Flux volumes and areas for water transfers

      FAREA = FAREAXY(I,J)
      
      IF( (VEGTYP.EQ.43).OR.(VEGTYP.EQ.44)) THEN
        VOL_FLUX_RUNON = VOL_FLUX_RUNONXY(I-1,J)
      ELSE
        VOL_FLUX_RUNON = 0.
      END IF 
      ...
      IF (VEGTYP.EQ.47) THEN 
        DO IZ = 1,NSOIL
           VOL_FLUX_SM(IZ) = VOL_FLUX_SMXY(I-1,IZ,J)
        END DO
      ELSE 
...
! The TRANSFER_MP_PARAMETERS has been modified slightly due to changes in the
! permeable pavements. The pavements will be based on the Sand, but with a different saturated hydraulic conductivity
! (Higher)
...
!         ZWT    = undefined_value
    ...
    ELSE
         ...
         CALL NOAHMP_SFLX (parameters, &
            ...
            FVEG    , FVGMAX  , VEGTYP  , ICE     , IST     , CROPTYPE, & ! IN : Vegetation/Soil characteristics !edited by G. Aaron Alexander
	    ...
	    PRCPCONV, PRCPNONC, PRCPSHCV, PRCPSNOW, PRCPGRPL, PRCPHAIL, & ! IN : Forcing
            ...
            GECROS1D,SMC_INTERMEDIATE, SH2O_INTERMEDIATE,               & ! IN/OUT :
            Z0WRF   ,                                                   &
            FSA     , FSR     , FIRA    , FSH     , SSOIL   , FCEV    , & ! OUT :
            FGEV    , FCTR    , ECAN    , ETRAN   , ESOIL   , TRAD    , & ! OUT :
            TGB     , TGV     , T2MV    , T2MB    , Q2MV    , Q2MB    , & ! OUT :
            RUNSF   , RUNONSF ,  RUNSB   , APAR    , PSN     , SAV     , SAG     , & ! OUT : !changed by Aaron A.
	    ...
	    CHLEAF  , CHUC    , CHV2    , CHB2    , FPICE   , PAHV    , &
            ...
            , NSOIL_GR, DETENTION_STORAGE,FAREA,VOL_FLUX_SM,VOL_FLUX_RUNON )            ! OUT :
...
! INPUT/OUTPUT
      ...
      IF (VEGTYP.EQ.41) THEN !Added by Aaron Alexander
        SMOIS(I+1, 1:NSOIL, J ) = SMC_INTERMEDIATE(1:NSOIL)
        SH2O( I+1, 1:NSOIL, J ) = SH2O_INTERMEDIATE(1:NSOIL)

        SMOIS(I, 1:NSOIL, J ) = SMC(1:NSOIL)
        SH2O( I, 1:NSOIL, J ) = SMH2O(1:NSOIL)

      ELSEIF (VEGTYP.EQ.44) THEN !Added by Aaron Alexander (save the permeable pavement to a different area)
        SMOIS_PER_PAVEMENT(I, 1:NSOIL, J) = SMC(1:NSOIL)
        SH2O_PER_PAVEMENT(I, 1:NSOIL, J) = SMH2O(1:NSOIL)
      ...
      !       SMOIS    (I,      1:NSOIL,J)  = SMC   (      1:NSOIL)
       ...
       IF (VEGTYP.EQ.44) THEN
        TSLB_PER_PAVEMENT( I, 1:NSOIL, J) = STC( 1:NSOIL)
      ELSE
      ...
      VOL_FLUX_RUNONXY(I,J) = VOL_FLUX_RUNON
      VOL_FLUX_SMXY(I,1:NSOIL,J) = VOL_FLUX_SM
...
! OUTPUT
             ...
             FVEGXY   (I,J)                = FVEGMP
             RUNSFXY  (I,J)                = RUNSF
             RUNONSFXY(I,J)                = RUNONSF !added by Aaron A.
             RUNSBXY  (I,J)                = RUNSB
             ECANXY   (I,J)                = ECAN
             EDIRXY   (I,J)                = ESOIL
             ETRANXY  (I,J)                = ETRAN
             ...
             RB                            = MAX(RB, 0.0)
             BTRANI_output(I,1:NSOIL,J)    = BTRANI_dummy(1:NSOIL) !Added by Aaron A.
...
! ----------------------------------------------------------------------
! Adjust the parameter definitions associated with the soil IF the vegetation type
! is equal to permeable pavements. This soil type is based on the baseline soil
! type within the WRF SOIL.TBL, but has a higher saturated hydraulic conductivity

IF(VEGTYPE.EQ.44) THEN !Permeable Pavement Added by Aaron A. 
    do isoil = 1, 2
      parameters%BEXP(isoil)   = BEXP_TABLE   (1)
      parameters%DKSAT(isoil)  = 1.0E-4
      parameters%DWSAT(isoil)  = DWSAT_TABLE  (1)
      parameters%PSISAT(isoil) = PSISAT_TABLE (1)
      parameters%QUARTZ(isoil) = QUARTZ_TABLE (1)
      parameters%SMCDRY(isoil) = SMCDRY_TABLE (1)
      parameters%SMCMAX(isoil) = 0.4
      parameters%SMCREF(isoil) = SMCREF_TABLE (1)
      parameters%SMCWLT(isoil) = SMCWLT_TABLE (1)
    end do
ELSE IF(VEGTYPE.EQ.46) THEN ! Green Roof Added by Aaron A. 
    do isoil = 1, size(soiltype)
      parameters%BEXP(isoil)   = BEXP_TABLE   (4)
      parameters%DKSAT(isoil)  = DKSAT_TABLE  (4)
      parameters%DWSAT(isoil)  = DWSAT_TABLE  (4)
      parameters%PSISAT(isoil) = PSISAT_TABLE (4)
      parameters%QUARTZ(isoil) = QUARTZ_TABLE (5)
      parameters%SMCDRY(isoil) = 0.1
      parameters%SMCMAX(isoil) = 0.4
      parameters%SMCREF(isoil) = 0.335
      parameters%SMCWLT(isoil) = 0.1
...
! ----------------------------------------------------------------------
    ...
    IF((parameters%URBAN_FLAG).OR.(VEGTYPE.EQ.41))THEN  ! Hardcoding some urban parameters for soil
...
!------------------------------------------------------------------------------------------
  ...
  USE module_ra_gfdleta,  only: cal_mon_day
  USE NOAHMP_TABLES, ONLY: ISURBAN_TABLE, LOW_DENSITY_RESIDENTIAL_TABLE, HIGH_DENSITY_RESIDENTIAL_TABLE, HIGH_INTENSITY_INDUSTRIAL_TABLE
...
!
!============================================================================================
   SUBROUTINE NOAHMP_MOSAIC_INIT(XLAND, SNOW , SNOWH , CANWAT , ISLTYP ,   IVGTYP, XLAT, ISWATER, ISICE, &
...
!jref:end
       ...
       ims,ime, jms,jme, kms,kme,                &
       its,ite, jts,jte, kts,kte,                &
       LANDUSEF, LANDUSEF2, NLCAT, noahmp_mosaic_scheme,                     &      ! Added by Aaron A. **IMPORTANT FOR THE FOR LOOPS OF FRACTIONAL LAND USE
       ...
       TRL_URB3D_mosaic, TBL_URB3D_mosaic, TGL_URB3D_mosaic,                 &      ! Added by Aaron A.
       SH_URB2D_mosaic, LH_URB2D_mosaic,                                     &      ! Added by Aaron A.
...
! From here on, we define the important variables that are needed to re-catagorize the landuse data sets
  ...
  INTEGER, INTENT(IN) :: noahmp_mosaic_scheme                                       !This tells us that the mosaic scheme is active
...
!variables for 2d urban model
  ...
  REAL, DIMENSION( ims:ime, 1:mosaic_cat, jms:jme ) , OPTIONAL, INTENT(INOUT):: TS_RUL2D_mosaic
  ...
  if(iopt_crop.eq.2)then !check added by Aaron Alexander
    CALL wrf_error_fatal('NOAHMP MOSAIC DOES NOT SUPPORT GECROS MODEL FOR CROPS AT THIS TIME')
  endif

  if(iopt_run.eq.5)then !check added by Aaron Alexander
    CALL wrf_error_fatal('NOAHMP MOSAIC DOES NOT SUPPORT SHALLOW GROUNDWATER MODEL AT THIS TIME')
  endif
  !===========================================================================
  ! CHOOSE THE TILES
  !===========================================================================
  ...
  !===========================================================================
     ...
     DO i = its,itf
        DO j = jts,jtf
            ...
            lfmassxy_mosaic(i,mosiac_i,j)=lfmassxy(i,j)
           ...
           DO snow_layer=1,7
            zsnsoxy_mosaic(i,7*(mosaic_i-1)+snow_layer,j)=zsnsoxy(i,snow_layer,j)
           ENDDO

           DO top_layer=1,3
            tsnoxy_mosaic(i,3*(mosaic_i-1)+top_layer,j)=tsnoxy(i,snow_layer,j)
            snicexy_mosaic(i,3*(mosaic_i-1)+top_layer,j)=snicexy(i,snow_layer,j)
            snliqxy_mosaic(i,3*(mosaic_i-1)+top_layer,j)=snliqxy(i,snow_layer,j)
           ENDDO
