...
module module_NoahMP_hrldas_driver
  ...
  USE module_sf_noahmpdrv, only: noahmp_init, noahmplsm, noahmp_urban, groundwater_init, noahmp_mosaic_init !Edited by Aaron Alexander
...
! OUT (with no Noah LSM equivalent) (as defined in WRF)
  ...
  REAL,    ALLOCATABLE, DIMENSION(:,:)    ::  RUNONSFXY ! surface runon  [mm/s] ! Added by Aaron A.
...
!------------------------------------------------------------------------
  ...
  REAL,   ALLOCATABLE, DIMENSION(:,:,:)  :: lai_mosaic
  ...
  !add root water uptake to the model output
  REAL,   ALLOCATABLE, DIMENSION(:,:,:) :: BTRANI_output
...
!------------------------------------------------------------------------
! Additional OUTPUTS For GREEN ROOF and Permeable Pavements - Aaron A.
!------------------------------------------------------------------------
  REAL,  ALLOCATABLE, DIMENSION(:,:,:) :: TSLB_PER_PAVEMENT
  REAL,  ALLOCATABLE, DIMENSION(:,:,:) :: SMOIS_PER_PAVEMENT
  REAL,  ALLOCATABLE, DIMENSION(:,:,:) :: SH2O_PER_PAVEMENT
  REAL,  ALLOCATABLE, DIMENSION(:,:) :: DETENTION_STORAGEXY
  REAL,  ALLOCATABLE, DIMENSION(:,:) :: FAREAXY 
  REAL,  ALLOCATABLE, DIMENSION(:,:) :: VOL_FLUX_RUNONXY
  REAL,  ALLOCATABLE, DIMENSION(:,:,:) :: VOL_FLUX_SMXY

  !REAL,  ALLOCATABLE, DIMENSION(:,:,:) :: TSLB_PER_PAVEMENT_mosaic
  !REAL,  ALLOCATABLE, DIMENSION(:,:,:) :: SMOIS_PER_PAVEMENT_mosaic
  !REAL,  ALLOCATABLE, DIMENSION(:,:,:) :: SH2O_PER_PAVEMENT_mosaic
...
!---------------------------------------------------------------------
  ...
  !!Namelist modifications added by Aaron Alexander
  ...
  REAL, DIMENSION(100) :: farea_input       ! depth to soil interfaces from namelist [m]
  !!End Aaron Alexander Modifications
  ...
  real               :: urban_atmosphere_thickness = 15.0
  ...
  namelist / NOAHLSM_OFFLINE /    &
       ...
       noahmp_mosaic_scheme, geogrid_file_name_for_mosaic,number_mosaic_catagories,number_land_use_catagories,farea_input
...
!---------------------------------------------------------------------
  ...
  !!Check written by Aaron Alexander for mosaic
  if (noahmp_mosaic_scheme /= 0 .and. noahmp_mosaic_scheme /= 1) then
        write(*, *)
        write(*, '(" ***** Namelist error: ******************************************************")')
        write(*, '(" ***** ")')
        write(*, '(" *****       Noahmp mosaic option must be either 0 (off) or 1 (on)")')
        write(*, *)
        stop
  endif
  ...
  if (noahmp_mosaic_scheme == 1 ) then
     if (number_land_use_catagories == -999) then
        ...
        write(*, '(" ***** Namelist error: ******************************************************")')
        write(*, '(" ***** ")')
        write(*, '(" *****       Noahmp mosaic requires you to set NUMBER_LAND_USE_CATAGORIES namlist variable")')
        write(*, *)
        stop
     endif
  endif
...
!INOUT (Green Noah - MP Additions)
  ALLOCATE ( TSLB_PER_PAVEMENT  (XSTART:XEND,1:NSOIL,YSTART:YEND) ) !soil temperature for pervious pavements [K]
  ALLOCATE ( SMOIS_PER_PAVEMENT (XSTART:XEND,1:NSOIL,YSTART:YEND) ) !volumetric soil moisture [m3/m3] for pervious pavements
  ALLOCATE ( SH2O_PER_PAVEMENT  (XSTART:XEND,1:NSOIL,YSTART:YEND) ) !volumetric liquid soil moisture [m3/m3] for pervious pavements
  ALLOCATE ( DETENTION_STORAGEXY(XSTART:XEND,        YSTART:YEND) ) !detetention storage of greeen roof out flow [M]
  ALLOCATE ( FAREAXY(XSTART:XEND, YSTART:YEND) )
  ALLOCATE ( VOL_FLUX_RUNONXY(XSTART:XEND, YSTART:YEND) )
  ALLOCATE ( VOL_FLUX_SMXY(XSTART:XEND, 1:NSOIL, YSTART:YEND) )
! OUT (with no Noah LSM equivalent) (as defined in WRF)
  ...
  ALLOCATE ( RUNONSFXY (XSTART:XEND,YSTART:YEND) )  ! surface runon [mm/s] !added by Aaron A.
...
!IF(SF_URBAN_PHYSICS == 1 ) THEN  ! single layer urban model
  ...
  ! ALLOCATE ( frc_urb2d      (XSTART:XEND,                 YSTART:YEND) )  !
  ! ALLOCATE ( utype_urb2d    (XSTART:XEND,                 YSTART:YEND) )  ! Allocated already Aaron A. 2 Oct 2020
...
IF(noahmp_mosaic_scheme == 1) THEN
  ...
  ALLOCATE ( LANDUSEF2        (XSTART:XEND,1:NUMBER_LAND_USE_CATAGORIES,YSTART:YEND) )
  ALLOCATE ( mosaic_cat_index (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( TSK_mosaic       (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( TMN_mosaic       (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( CANWAT_mosaic    (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( SNOW_mosaic      (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( SNOWH_mosaic     (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( tvxy_mosaic      (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( tgxy_mosaic      (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( canicexy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( canliqxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( eahxy_mosaic     (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( tahxy_mosaic     (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( cmxy_mosaic      (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( chxy_mosaic      (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( fwetxy_mosaic    (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( sneqvoxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( alboldxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( qsnowxy_mosaic   (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( wslakexy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( zwtxy_mosaic     (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( waxy_mosaic      (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( wtxy_mosaic      (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( lfmassxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( rtmassxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( stmassxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( woodxy_mosaic    (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( grainxy_mosaic   (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( gddxy_mosaic     (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( stblcpxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( fastcpxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( xsaixy_mosaic    (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( lai_mosaic       (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( t2mvxy_mosaic    (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( t2mbxy_mosaic    (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( chstarxy_mosaic  (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )

  ALLOCATE ( isnowxy_mosaic    (XSTART:XEND,1:number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( zsnsoxy_mosaic    (XSTART:XEND,1:7*number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( tsnoxy_mosaic     (XSTART:XEND,1:3*number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( snicexy_mosaic    (XSTART:XEND,1:3*number_mosaic_catagories,  YSTART:YEND) )
  ALLOCATE ( snliqxy_mosaic    (XSTART:XEND,1:3*number_mosaic_catagories,  YSTART:YEND) )

  ALLOCATE ( TSLB_mosaic       (XSTART:XEND,1:NSOIL*number_mosaic_catagories, YSTART:YEND) )
  ALLOCATE ( SMOIS_mosaic      (XSTART:XEND,1:NSOIL*number_mosaic_catagories, YSTART:YEND) )
  ALLOCATE ( SH2O_mosaic       (XSTART:XEND,1:NSOIL*number_mosaic_catagories, YSTART:YEND) )
  ...
  !-----
  ! Additional variables from Permeable Pavements- Aaron A.
  !-----

  ! ALLOCATE ( TSLB_PER_PAVEMENT_mosaic  (XSTART:XEND, 1:NSOIL*number_mosaic_catagories, YSTART:YEND) )
  ! ALLOCATE ( SMOIS_PER_PAVEMENT_mosaic (XSTART:XEND, 1:NSOIL*number_mosaic_catagories, YSTART:YEND) )
  ! ALLOCATE ( SH2O_PER_PAVEMENT_mosaic  (XSTART:XEND, 1:NSOIL*number_mosaic_catagories, YSTART:YEND) )

  ENDIF

  !Allocate btrani_output for use later Aaron A.
  ALLOCATE ( BTRANI_output (XSTART:XEND,1:NSOIL,YSTART:YEND))
  ...
  BTRANI_output = undefined_real !added by Aaron A.
  DETENTION_STORAGEXY = 0.0
  VOL_FLUX_RUNONXY = undefined_real
  VOL_FLUX_SMXY = undefined_real 
  ! only for current vector mode
  DO I = XSTART,XEND 
    DO J = YSTART,YEND 
     FAREAXY(I,J) = farea_input(I+J)
    END DO 
  END DO   
...
! Must still call NOAHMP_INIT even in restart to set up parameter arrays (also done in WRF)
     ...
     CALL NOAHMP_INIT(    LLANDUSE,     SNOW,    SNOWH,   CANWAT,   ISLTYP,   IVGTYP, XLAT, &   ! call from WRF phys_init
         ...
         IF(SF_URBAN_PHYSICS > 0 ) THEN  !urban
          ...
          ENDIF
    !------------------------------Initialize the permeable pavements to be an exact copy of the NOAH-MP INIT
    !added by Aaron A.
    TSLB_PER_PAVEMENT(:,:,:) = TSLB
    SMOIS_PER_PAVEMENT(:,:,:) = SMOIS
    SH2O_PER_PAVEMENT(:,:,:) = SH2O

  else
    !If for mosaic options (branched if and else statement that uses mosaic flag)
...
!     SNOW = SNOW * 1000.    ! Convert snow water equivalent to mm. MB: remove v3.7
    ...
    else if(noahmp_mosaic_scheme.eq.1) then !this is the switch for the noahmp mosaic option
      ...
      CALL NOAHMP_MOSAIC_INIT(XLAND, SNOW,    SNOWH,   CANWAT,   ISLTYP,   IVGTYP, XLAT, ISWATER, ISICE,&   ! call from WRF phys_init
                ...
                STMASSXY,   WOODXY, STBLCPXY, FASTCPXY,   XSAIXY, LAI,                    &
                 GRAINXY,    GDDXY,                                                   &
                  ...
                  TS_RUL2D_mosaic )                                                            ! Added by Aaron A.
      WRITE(*,*) ISWATER
      WRITE(*,*)'SUCCES AT RUNNING NOAHMP_MOSAIC_INIT ADDED BY AARON A.'
      WRITE(*,*)'SUPER HAPPY'
   stop
		 ...
		 WOODXY, STBLCPXY, FASTCPXY,      LAI,    XSAIXY,    TAUSSXY, &
	        SMOISEQ, SMCWTDXY,DEEPRECHXY,  RECHXY,   GRAINXY,      GDDXY,PGSXY, & ! IN/OUT Noah MP only
	         ...
	         T2MVXY,   T2MBXY,   Q2MVXY,   Q2MBXY,                        &
                 TRADXY,    NEEXY,    GPPXY,    NPPXY,    FVEGXY,    RUNSFXY,  RUNONSFXY, & !Aaron A. added runonsfxy
		  ...
		  GHBXY,    IRGXY,    IRCXY,    IRBXY,      TRXY,      EVCXY, &
	       CHLEAFXY,   CHUCXY,   CHV2XY,   CHB2XY,        RS,      BTRANI_output,  & !Added by Aaron A.
               TSLB_PER_PAVEMENT, SMOIS_PER_PAVEMENT, SH2O_PER_PAVEMENT,      & ! IN/OUT Green Noah-MP Added by Aaron A.
               DETENTION_STORAGEXY,  FAREAXY, VOL_FLUX_RUNONXY, VOL_FLUX_SMXY, &                                           
...
! Output for history
     OUTPUT_FOR_HISTORY: if (output_timestep > 0) then
        ...
        ! Grid water budget terms - in addition to above
              ...
              call add_to_output(RUNSFXY    , "RUNSFXY" ,  "rate or surface runoff"              , "mm s{-1}"              )!added by aaron a.
              call add_to_output(RUNONSFXY  , "RUNONSFXY"," rate of surface runon "              , "mm s{-1}"              )!added by aaron a.
              ...
              call add_to_output(DETENTION_STORAGEXY, "DETENTION_STORAGEXY", "Green Roof Detention storage" , "m"         )
        ...
        ! Needed to close the bare ground energy budget
              ...
              call add_to_output(BTRANI_output, "BTRANI"  , "Dimensionless Root Water Uptake: Multply by ETRAN to get Root Water Uptake", "UNITLESS","SOIL" ) !added by Aaron A.
        ...
        ! Soil Terms added by Aaron - 3D
              call add_to_output(SMOIS_PER_PAVEMENT, "SOIL_M_PER_PAVEMENT", "Volumetric Soil Moisture Permeable Pavement", "m3 m-3", "SOIL")
              call add_to_output(SH2O_PER_PAVEMENT, "SOIL_W_PER_PAVEMENT", "Liquid Volumetric Soil Moisture Permeable Pavement", "m3 m-3", "SOIL")
              call add_to_output(TSLB_PER_PAVEMENT, "SOIL_T_PER_PAVEMENT", "Soil Temperature Permeable Pavement", "K", "SOIL")
              call add_to_output( SMR_URB3D  ,     "SMR_URB3D", "Liquid Volumetric Soil Moisture Green Roof", "m3 m-3", "SOIL") 
        ! Snow - 2D terms
        ...
        ! urban model ouptut
              call add_to_output(     SH_URB2D  ,      "SH_URB2D" ,'Sensible Heat Urban', "W m{-2}")
              call add_to_output(     LH_URB2D  ,      "LH_URB2D" ,'Latent Heat Urban',"W m{-2}")
              call add_to_output(      G_URB2D  ,       "G_URB2D" ,'Ground Heat Urban',"W m{-2}")
              call add_to_output(     RN_URB2D  ,      "RN_URB2D" ,'Net Radiation Urban',"W m{-2}")
              call add_to_output(     TS_URB2D  ,      "TS_URB2D" ,'Surface Temperature',"K")
              call add_to_output(FLXHUMR_URB2D  , "FLXHUMR_URB2D", "Roof Latent Heat","W m{-2}")
         ! Carbon allocation model
	    IF(RUNOFF_OPTION == 5) THEN
