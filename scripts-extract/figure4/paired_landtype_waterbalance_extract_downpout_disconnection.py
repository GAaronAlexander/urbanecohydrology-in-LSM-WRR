...
save_loc = '../../intermediate-data/figure4/downspout_disconnection/'
...
analysis_range = pd.date_range(start='2020-03-31 18:00:00',end='2020-10-31 18:00:00', freq='5T')
...
SFC_RUNOFF_stan = data_standard.SFCRNOFF.sel(Time=analysis_range[-1]) - data_standard.SFCRNOFF.sel(Time='2020-04-30 18:00:00')

#create loop over the percentage

data_extracted = np.zeros((8,61))
...
for i, file in enumerate(data_directory_list):
	...
	SFC_RUNOFF = data_loop.SFCRNOFF.sel(Time=analysis_range[-1]) - data_loop.SFCRNOFF.sel(Time='2020-04-30 18:00:00')

	
	UNDER_RUNOFF = data_loop.UGDRNOFF.sel(Time=analysis_range[-1]) - data_loop.UGDRNOFF.sel(Time='2020-04-30 18:00:00')
	...
	data_extracted[2,i] = SFC_RUNOFF[0,1]*Percentage_turfgrass

	# ET from yard
	data_extracted[3,i] = ETRAN[0,1]*Percentage_turfgrass
	# ET from tree
	data_extracted[4,i] = ETRAN[0,0]*percentage_pavement

	# normal soil moisture
	data_extracted[5,i] = SOIL_M_norm[1]*Percentage_turfgrass
	# underground runoff
	data_extracted[6,i] = UNDER_RUNOFF[0,1]*Percentage_turfgrass 

np.savetxt(save_loc+'turfgrass_2020.csv',data_extracted,delimiter=',')
