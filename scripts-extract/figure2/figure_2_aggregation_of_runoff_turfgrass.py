...
data_turf = xr.open_dataset(data_location+'turfgrass_with_some_disconnect_00'+'/201804010000.LDASOUT_DOMAIN1')
...
for i,ll in enumerate(data_directory_list):
	...
	print(Percentage_turfgrass,percentage_pavement,percentage_pavement_dis)
	...
	data_standard_rain = data_standard.RUNSFXY[:,0,0]*rain_windows*300*percentage_pavement + data_loop.RUNSFXY[:,0,0]*rain_windows*300*percentage_pavement_dis + data_loop.RUNSFXY[:,0,1]*rain_windows*300*Percentage_turfgrass
	...
	data_aggrigate_things = data_aggrigate_things[0:125]
	


	data_to_save[i,:] = data_aggrigate_things[:]

np.savetxt('../indermediate-data/figure_2/three_season_turfgrass_12hourwindow.csv',data_to_save,delimiter=',')
