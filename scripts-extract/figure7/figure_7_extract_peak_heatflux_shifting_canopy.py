...
data_location = '/Volumes/Untitled/Integragting_LID_into_LSM/three_season_long_simulations/three_season_tree_shift/'
...
save_loc = '../intermediate-data/figure7/extract_data_tree_shift/'
...
data_extracted_HFX = np.ones((6,71))
data_extracted_LH = np.ones((6,71))
...
data_turf = xr.open_dataset(data_location+'turfgrass_inputs_threeseason'+'/201804010000.LDASOUT_DOMAIN1')
...
for i,n in enumerate(data_directory_list):
	...
	HFX_data_loop = data_stan.HFX.isel(west_east=0)* percentage_pavement  + data_loop.HFX.isel(west_east=1)*percentage_over_yard + data_loop.HFX.isel(west_east=0)*Percentage_over_pavement_tree + data_turf.HFX.isel(west_east=1)*percentae_turfgrass
	LH_data_loop = data_stan.LH.isel(west_east=0)* percentage_pavement  + data_loop.LH.isel(west_east=1)*percentage_over_yard + data_loop.LH.isel(west_east=0)*Percentage_over_pavement_tree +data_turf.LH.isel(west_east=1)*percentae_turfgrass
...
np.savetxt(save_loc+'HFX_extracted_max_min_tree_shift.csv',data_extracted_HFX,delimiter=',')
np.savetxt(save_loc+'LH_extracted_max_min_tree_shift.csv',data_extracted_LH,delimiter=',')