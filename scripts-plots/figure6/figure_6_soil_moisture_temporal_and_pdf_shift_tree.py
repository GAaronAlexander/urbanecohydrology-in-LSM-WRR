...
data_loc2 = '/Users/aaronalexander/Google Drive/My Drive/dissertation_chapter1_data/raw_outputs_from_cheyenne/three_season_shifting_tree/'
...
data_turf = xr.open_dataset(data_loc2 + 'turf_grass_three_season/201804010000.LDASOUT_DOMAIN1')

data_plotwise = xr.open_dataset(data_loc2+'share_tree_shifting_treecover_percentage_10/201804010000.LDASOUT_DOMAIN1')
data_plotwise_shift = xr.open_dataset(data_loc2+'share_tree_shifting_treecover_percentage_50/201804010000.LDASOUT_DOMAIN1')
data_plotwise_shift2 = xr.open_dataset(data_loc2+'share_tree_shifting_treecover_percentage_70/201804010000.LDASOUT_DOMAIN1')
...
soil_m_plotwise_shift2 = data_plotwise_shift2.SOIL_M[1:,0,0]*100/2000 + data_plotwise_shift2.SOIL_M[1:,0,1]*400/2000 + data_plotwise_shift2.SOIL_M[1:,0,2]*600/2000 + data_plotwise_shift2.SOIL_M[1:,0,3]*1000/2000

soil_m_turf = data_turf.SOIL_M[1:,0,0]*100/2000 + data_turf.SOIL_M[1:,0,1]*400/2000 + data_turf.SOIL_M[1:,0,2]*600/2000 + data_turf.SOIL_M[1:,0,3]*1000/2000
...
soil_m_turf = soil_m_turf.assign_coords({'Time':dates})
...
ax1.plot(soil_m_plotwise.Time,soil_m_plotwise[:,1]*0.9 + soil_m_turf[:,1]*0.1,color='#75BD81',linewidth=7.0)

ax1.plot(soil_m_plotwise_shift.Time, soil_m_plotwise_shift2[:,1]*0.3 + soil_m_turf[:,1]*0.7,color='#26A33A',linewidth=7.0)
...
# plt.savefig(save_loc+'TEST_shifttree_silt_loam_soilmoisture_pdf.png',dpi=300)
# plt.close()
...
soil_m_plotwise_shift2 = soil_m_plotwise_shift.drop_sel(Time=date_drop1)
soil_m_turf = soil_m_turf.drop_sel(Time=date_drop1)
...
soil_m_plotwise_shift2 = soil_m_plotwise_shift2.drop_sel(Time=date_drop2)
soil_m_turf = soil_m_turf.drop_sel(Time=date_drop2)

soil_m_standard = soil_m_standard.dropna('Time')
soil_m_plotwise = soil_m_plotwise.dropna('Time')
soil_m_plotwise_shift2 = soil_m_plotwise_shift2.dropna('Time')
soil_m_turf = soil_m_turf.dropna('Time')
...
## plotwise 10% turf, 90% tree
kde_10 = KernelDensity(kernel="gaussian", bandwidth=0.01).fit(soil_m_plotwise[:,1].values.reshape(-1,1)*0.9+soil_m_turf[:,1].values.reshape(-1,1)*0.1)
...
ax2.plot(X_plot,CDF_10,color='#75BD81',linewidth=7.0)

## plotwise 70% turf, 30% Tree
kde_35 = KernelDensity(kernel="gaussian", bandwidth=0.01).fit(soil_m_plotwise_shift2[:,1].values.reshape(-1,1)*.3 + soil_m_turf[:,1].values.reshape(-1,1)*0.7)
log_dens_35 = kde_35.score_samples(X_plot.reshape(-1,1))
CDF_35 =np.exp(log_dens_35)*(X_plot[1] - X_plot[0])

ax2.plot(X_plot,CDF_35,color='#26A33A',linewidth=7.0)
...
plt.savefig(save_loc+'soil_moisutre_shifttree_silt_PDF.png',dpi=500)
...
# plt.tight_layout()
# plt.savefig(save_loc+'AGU_soil_moisture_tree_share_pdf.png',dpi=300)
# # plt.show()
